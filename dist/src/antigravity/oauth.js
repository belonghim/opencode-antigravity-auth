...
import { ANTIGRAVITY_CLIENT_ID, ANTIGRAVITY_CLIENT_SECRET, ANTIGRAVITY_REDIRECT_URI, ANTIGRAVITY_SCOPES, ANTIGRAVITY_ENDPOINT_FALLBACKS, ANTIGRAVITY_LOAD_ENDPOINTS, getAntigravityHeaders, GEMINI_CLI_HEADERS, GEMINI_CLI_CLIENT_ID, GEMINI_CLI_CLIENT_SECRET, GEMINI_CLI_SCOPES, } from "../constants";
 ...
 */
export async function authorizeGeminiCli(redirectUri, loginHint) {
    const pkce = (await generatePKCE());
    const url = new URL("https://accounts.google.com/o/oauth2/v2/auth");
 ...
 */
export async function exchangeGeminiCli(code, verifier, redirectUri) {
    try {
        const tokenResponse = await fetch("https://oauth2.googleapis.com/token", {
            method: "POST",
            headers: {
