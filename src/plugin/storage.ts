...
// proper-lockfile removed

// Simple mutex to replace proper-lockfile
const _locks = new Map<string, Promise<void>>();
const _lockfile = {
  lock: async (path: string, _options?: unknown): Promise<() => Promise<void>> => {
    while (_locks.has(path)) await _locks.get(path);
    let resolve!: () => void;
    _locks.set(path, new Promise<void>(r => { resolve = r; }));
    return async () => { _locks.delete(path); resolve(); };
  }
};
...
async function withFileLock<T>(path: string, fn: () => Promise<T>): Promise<T> {
  ...
  try {
    release = await _lockfile.lock(path, LOCK_OPTIONS);
    return await fn();
  } finally {
    if (release) {
      try {
